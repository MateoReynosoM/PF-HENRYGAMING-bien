...
routeProductCart.post('/',async (req, res, next)=>{
    ...
    if(!idUser && !idProduct) return res.send({message: 'No se enviaron los datos correctos'})
    try {
        ...
        if(!createdCart && !createdCartProduct){
            ...
            await cart.update({total: amount? (cart.total + (product.price * amount) ): (cart.total + product.price)});
            await cartProduct.update({amount: amount ? (cartProduct.amount + amount):(cartProduct.amount + 1)});
            await cart.save();
        ...
        if(user && product && cart && cartProduct && createdCart && createdCartProduct){
